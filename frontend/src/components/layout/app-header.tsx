...
type HeaderProps = { navItems: NavItem[]; activePath?: string; };
...
export function AppHeader({ navItems, activePath }: HeaderProps) {
  ...
  useEffect(() => {
    if (pathname === "/search") setSearchValue(searchParams.get("q") || "");
    ...
    <header className="h-16 border-b bg-card border-border flex items-center px-4 gap-4">
      ...
      <Link href="/" className="flex items-center gap-2 font-semibold text-lg text-foreground">
        <div className="relative h-8 w-8 shrink-0">
          <Image src="/images/ute_logo.png" alt="UTE Logo" fill sizes="32px" className="object-contain" priority />
        </div>
      ...
      <div className="flex items-center gap-1">
        ...
        {authReady && (
          <>
            {isAuthenticated && <div className="hidden sm:block"><TrustPointsBadge /></div>}
            {isAuthenticated && <div className="hidden sm:block"><EventMenu /></div>}
            {isAuthenticated && <div className="hidden sm:block"><NotificationMenu /></div>}
            {isAuthenticated ? (
              <UserMenu />
            ) : (
              <>
