  ...
  return (
    <DropdownMenu open={dropdown.open} onOpenChange={dropdown.setOpen}>
      <DropdownMenuTrigger asChild>
        <Button variant="ghost" size="sm" className="h-9 w-9 rounded-full p-0 hover:bg-muted" aria-label="User menu">
          <Avatar className="h-9 w-9">
            <AvatarImage src={profile?.avatarUrl || undefined} alt={profile?.fullName || 'User'} />
            <AvatarFallback className="bg-muted text-muted-foreground">
              {profile?.fullName ? getInitials(profile.fullName) : <User size={18} />}
            </AvatarFallback>
          </Avatar>
        </Button>
      </DropdownMenuTrigger>
      <DropdownMenuContent align="end" className="w-56">
        ...
        <DropdownMenuItem asChild>
          <Link href="/profile" className="cursor-pointer">
            <User size={16} className="mr-2" />{tNav('profile')}
          </Link>
        </DropdownMenuItem>
        <DropdownMenuSeparator />
        <DropdownMenuItem
