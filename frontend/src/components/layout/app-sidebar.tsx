...
type SidebarProps = { navItems: (NavItem | AdminNavItem)[]; activePath?: string };
...
export function AppSidebar({ navItems, activePath }: SidebarProps) {
  ...
  const handleCreateDocument = () => {
    if (authenticated) router.push("/documents/upload");
      ...
      <aside className="hidden md:flex w-72 shrink-0 border-r bg-card border-border h-full overflow-hidden flex-col">
        <ScrollArea className="w-full flex-1">
          <div className="p-4 space-y-4">
            {!isInAdminPanel && (
              <Button className="w-full rounded-full mt-2" onClick={handleCreateDocument}>
                {t('new-document')}
              </Button>
            )}
              ...
              {navItems.map(item => {
                ...
                return (
                  <Link key={item.href} href={item.href}>
                    <button className={`flex w-full items-center gap-3 px-3 py-2 text-left transition-colors ${isActive ? "bg-muted text-foreground font-medium" : "text-muted-foreground hover:bg-muted"}`}>
            ...
            {canAccessAdmin && (
              <div className="pt-4 border-t">
                {isInAdminPanel ? (
                  ...
                  <Link href="/admin">
                    <button className="flex w-full items-center gap-3 px-3 py-2 text-left transition-colors text-muted-foreground hover:bg-muted">
                      <Shield size={18} /><span>Quản trị</span>
                    </button>
                  </Link>
                )}
              </div>
        ...
        <div className="p-3">
          <Link href="/about">
            <button className={`flex w-full items-center gap-3 px-3 py-2 text-sm text-left transition-colors ${activePath === "/about" ? "bg-muted text-foreground font-medium" : "text-muted-foreground hover:bg-muted"}`}>
              <Info size={18} /><span>{tNav('about')}</span>
            </button>
          </Link>
        </div>
      </aside>
