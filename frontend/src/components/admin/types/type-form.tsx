...
import { getApiType } from "@/src/api";
...
export function TypeForm({ initialData, onSubmit, loading }: TypeFormProps) {
  const t = useTranslations("admin.types");
  const [formData, setFormData] = useState<TypeFormData>({ typeName: null });
  ...
  const searchTypes = useCallback(async (searchTerm: string) => {
    if (!searchTerm.trim()) { setMatchingTypes([]); setIsDuplicate(false); return; }
    setSearching(true);
    try {
      ...
      setIsDuplicate(filtered.some(item => item.typeName?.toLowerCase() === searchTerm.toLowerCase()));
    } catch { setMatchingTypes([]); setIsDuplicate(false); }
    finally { setSearching(false); }
  }, [initialData?.id]);

  useEffect(() => {
    if (initialData?.typeName && debouncedName === initialData.typeName) { setMatchingTypes([]); setIsDuplicate(false); return; }
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    await onSubmit({ typeName: formData.typeName || undefined });
  };
      ...
      <div className="space-y-3">
        <div>
          ...
          <div className="relative">
            <Input id="typeName" value={formData.typeName || ""} onChange={e => setFormData(prev => ({ ...prev, typeName: e.target.value }))} required disabled={isDisabled} className={`mt-1 ${isDuplicate ? "border-red-500 focus-visible:ring-red-500" : ""}`} placeholder={t("form.typeNamePlaceholder")} />
          ...
          {matchingTypes.length > 0 && !isDuplicate && (
            <div className="mt-2 p-2 bg-muted border border-border">
              <p className="text-xs text-muted-foreground mb-1">{t("form.similarTypes")}:</p>
              <ul className="space-y-0.5">{matchingTypes.map(type => <li key={type.id} className="text-sm text-foreground">• {type.typeName}</li>)}</ul>
            </div>
          )}
        </div>
      </div>
