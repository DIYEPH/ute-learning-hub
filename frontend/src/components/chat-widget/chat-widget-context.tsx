...
export function ChatWidgetProvider({ children }: { children: ReactNode }) {
  ...
  const openChat = useCallback((conversation: ConversationDto) => {
    ...
    setWidgets(prev => {
      ...
      if (existing) return prev.filter(w => w.id !== conversationId).concat({ ...existing, isMinimized: false });
      let newWidgets = [...prev];
      if (newWidgets.length >= MAX_WIDGETS) newWidgets = newWidgets.slice(1);
      return [...newWidgets, { id: conversationId, conversation, isMinimized: false, hasUnread: false }];
    });
  }, []);
  ...
  const toggleMinimize = useCallback((conversationId: string) => {
    setWidgets(prev => prev.map(w => w.id === conversationId ? { ...w, isMinimized: !w.isMinimized } : w));
  }, []);
  ...
  const markAsRead = useCallback((conversationId: string) => {
    setWidgets(prev => prev.map(w => w.id === conversationId ? { ...w, hasUnread: false } : w));
  }, []);

  const markAsUnread = useCallback((conversationId: string) => {
    setWidgets(prev => prev.map(w => w.id === conversationId ? { ...w, hasUnread: true } : w));
  }, []);
  ...
  return (
    <ChatWidgetContext.Provider value={{ widgets, openChat, closeChat, toggleMinimize, minimizeAll, markAsRead, markAsUnread, bringToFront }}>
      {children}
