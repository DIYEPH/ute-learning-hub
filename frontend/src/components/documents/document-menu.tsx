...
export function DocumentMenu({ onEdit, onDelete, onReport, disabled = false }: DocumentMenuProps) {
  ...
  return (
    <DropdownMenu>
      ...
      <DropdownMenuContent align="end" className="w-40">
        <DropdownMenuItem onSelect={event => handleSelect(event, onEdit)} disabled={!onEdit || disabled}>
          <Pencil className="h-4 w-4" />Chỉnh sửa
        </DropdownMenuItem>
        <DropdownMenuItem onSelect={event => handleSelect(event, onDelete)} disabled={!onDelete || disabled} variant="destructive">
          <Trash2 className="h-4 w-4" />Xóa
        </DropdownMenuItem>
        <DropdownMenuSeparator />
        <DropdownMenuItem onSelect={event => handleSelect(event, onReport)} disabled={!onReport || disabled}>
