...
export function DocumentCard({
  id, title, subjectName, thumbnailFileId, fileMimeType, tags, href, className,
  fileCount, usefulCount, notUsefulCount, totalViewCount, onEdit, onDelete, onReport, onSave
}: DocumentCardProps) {
  ...
  const ratingPercent = totalReviews > 0 ? Math.round((usefulCount || 0) / totalReviews * 100) : null;

  const content = (
    <div className={cn(
      "group relative flex h-full flex-col bg-card rounded-xl border border-border/50 shadow-sm transition-all duration-200 cursor-pointer overflow-hidden hover:shadow-md hover:border-primary/30 hover:-translate-y-0.5",
      className
    )}>
      {showMenu && (
        <div className="absolute right-2 top-2 z-10 opacity-0 group-hover:opacity-100 transition-opacity">
          <DocumentMenu onEdit={onEdit} onDelete={onDelete} onReport={onReport} />
        </div>
      )}
      ...
      <div className="px-3 pb-3 flex flex-col gap-1.5 flex-1">
        ...
        {subjectName && <p className="text-xs text-muted-foreground line-clamp-1">{subjectName}</p>}
        {tags && tags.length > 0 && (
          <div className="flex flex-wrap gap-1">
            {tags.slice(0, 2).map(tag => (
              <span key={tag} className="px-1.5 py-0.5 text-[10px] font-medium bg-muted text-muted-foreground rounded">
                {tag}
              </span>
            ))}
  ...
  if (!resolvedHref || resolvedHref === "#") return content;
  return <Link href={resolvedHref} className="block">{content}</Link>;
}
