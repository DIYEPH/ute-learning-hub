...
import { Filter, X, Loader2, ChevronDown, ChevronUp } from "lucide-react";
...
export default function SearchPage() {
    ...
    useEffect(() => { void loadFilterOptions(); }, []);
    ...
    const searchDocuments = async (pageNum: number, append: boolean) => {
        ...
        try {
            ...
            append ? setDocuments(prev => [...prev, ...items]) : setDocuments(items);
    ...
    const handleLoadMore = () => {
        if (hasMore && !loadingMore) void searchDocuments(page + 1, true);
    };
    ...
    return (
        <div className="flex flex-col lg:flex-row gap-6">
            <aside
                className={`shrink-0 transition-all ${showFilters
                    ...
                    >
                        <div className="space-y-2">
                            ...
                            <button
                                ...
                                className={`w-full text-left px-2 py-1.5 rounded text-sm transition-colors ${noSubject ? "bg-muted text-foreground" : "hover:bg-muted text-muted-foreground italic"
                                    }`}
                            >
                                {t("noSubjectFilter")}
                            </button>
...
function FilterSection({
    ...
    return (
        <div className="border-b border-border pb-3">
            <button onClick={onToggle} className="flex items-center justify-between w-full text-left py-1">
                <span className="text-sm font-medium text-foreground flex items-center gap-2">
                    {title}
                    {badge && (
                        <span className="px-1.5 py-0.5 rounded-full bg-primary/10 text-primary text-xs">{badge}</span>
                    )}
                </span>
                {expanded ? (
