...
import { getApiFaculty, getApiFacultyById, postApiFaculty, putApiFacultyById, deleteApiFacultyById } from "@/src/api";
import type { GetApiFacultyData, GetApiFacultyResponse, CreateFacultyCommand, UpdateFacultyCommandRequest, FacultyDetailDto } from "@/src/api/database/types.gen";
...
export function useFaculties() {
  ...
  const crud = useCrud<FacultyDetailDto, CreateFacultyCommand, UpdateFacultyCommandRequest, GetApiFacultyData["query"]>({
    ...
    delete: async id => { await deleteApiFacultyById({ path: { id } }); },
    errorMessages: {
  ...
  const checkNameExists = useCallback(async (name: string, excludeId?: string): Promise<boolean> => {
    try {
      ...
      return items.some(item => item.facultyName?.toLowerCase() === name.toLowerCase() && item.id !== excludeId);
    } catch { return false; }
  }, []);
  ...
  return {
    ...
    error: crud.error,
    uploadLogo, uploadingLogo, uploadError,
    checkNameExists,
  };
}