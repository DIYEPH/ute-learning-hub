...
import { getApiType, getApiTypeById, postApiType, putApiTypeById, deleteApiTypeById } from "@/src/api";
import type { GetApiTypeData, GetApiTypeResponse, CreateTypeCommand, UpdateTypeCommandRequest, TypeDto } from "@/src/api/database/types.gen";
...
export function useTypes() {
  const crud = useCrud<TypeDto, CreateTypeCommand, UpdateTypeCommandRequest, GetApiTypeData["query"]>({
    ...
    delete: async id => { await deleteApiTypeById({ path: { id } }); },
    errorMessages: {
  ...
  const checkNameExists = useCallback(async (name: string, excludeId?: string): Promise<boolean> => {
    try {
      ...
      return items.some(item => item.typeName?.toLowerCase() === name.toLowerCase() && item.id !== excludeId);
    } catch { return false; }
  }, []);
