...
import { getApiSubject, getApiSubjectById, postApiSubject, putApiSubjectById, deleteApiSubjectById } from "@/src/api";
import type { GetApiSubjectData, GetApiSubjectResponse, CreateSubjectCommand, UpdateSubjectCommandRequest, SubjectDto2 } from "@/src/api/database/types.gen";
...
export function useSubjects() {
  const crud = useCrud<SubjectDto2, CreateSubjectCommand, UpdateSubjectCommandRequest, GetApiSubjectData["query"]>({
    ...
    delete: async id => { await deleteApiSubjectById({ path: { id } }); },
    errorMessages: {
  ...
  const checkNameExists = useCallback(async (name: string, excludeId?: string): Promise<boolean> => {
    try {
      ...
      return items.some(item => item.subjectName?.toLowerCase() === name.toLowerCase() && item.id !== excludeId);
    } catch { return false; }
  }, []);
