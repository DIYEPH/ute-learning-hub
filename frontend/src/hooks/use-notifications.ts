...
import { getApiNotification, postApiNotification, putApiNotificationById, deleteApiNotificationById } from "@/src/api";
import type { GetApiNotificationData, GetApiNotificationResponse, CreateNotificationCommand, UpdateNotificationCommandRequest, NotificationDto } from "@/src/api/database/types.gen";
...
export function useNotifications() {
    const crud = useCrud<NotificationDto, CreateNotificationCommand, UpdateNotificationCommandRequest, GetApiNotificationData["query"]>({
        ...
        delete: async id => { await deleteApiNotificationById({ path: { id } }); },
        errorMessages: {
    ...
    const checkTitleExists = useCallback(async (title: string, excludeId?: string): Promise<boolean> => {
        try {
