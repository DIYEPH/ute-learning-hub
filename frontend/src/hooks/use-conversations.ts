...
import { getApiConversation, getApiConversationById, putApiConversationById, deleteApiConversationById } from "@/src/api";
import type { ConversationDto, ConversationDetailDto, PagedResponseOfConversationDto, GetApiConversationData, UpdateConversationCommandRequest } from "@/src/api/database/types.gen";
...
export function useConversations() {
    const crud = useCrud<ConversationDto, object, UpdateConversationCommandRequest, GetApiConversationData["query"]>({
        ...
        update: async (id, command) => {
            const response = await putApiConversationById({ path: { id }, body: command });
            return (response as unknown as { data: ConversationDto })?.data || response as ConversationDto;
        },
        delete: async id => { await deleteApiConversationById({ path: { id } }); },
        errorMessages: {
    ...
    // Fetch single by ID
    const fetchConversationById = useCallback(async (id: string): Promise<ConversationDetailDto | null> => {
        try {
    ...
    const checkNameExists = useCallback(async (name: string, excludeId?: string): Promise<boolean> => {
        try {
            ...
            return items.some(item => item.conversationName?.toLowerCase() === name.toLowerCase() && item.id !== excludeId);
        } catch { return false; }
    }, []);
