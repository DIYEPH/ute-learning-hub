...
import { getApiMajor, getApiMajorById, postApiMajor, putApiMajorById, deleteApiMajorById } from "@/src/api";
import type { GetApiMajorData, GetApiMajorResponse, CreateMajorCommand, UpdateMajorCommandRequest, MajorDetailDto } from "@/src/api/database/types.gen";
...
export function useMajors() {
  const crud = useCrud<MajorDetailDto, CreateMajorCommand, UpdateMajorCommandRequest, GetApiMajorData["query"]>({
    ...
    delete: async id => { await deleteApiMajorById({ path: { id } }); },
    errorMessages: {
  ...
  const checkNameExists = useCallback(async (name: string, excludeId?: string): Promise<boolean> => {
    try {
      ...
      return items.some(item => item.majorName?.toLowerCase() === name.toLowerCase() && item.id !== excludeId);
    } catch { return false; }
  }, []);
