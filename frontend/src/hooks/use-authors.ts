...
import { getApiAuthor, getApiAuthorById, postApiAuthor, putApiAuthorById, deleteApiAuthorById } from "@/src/api";
import type { AuthorDto, AuthorDetailDto, AuthorInput, UpdateAuthorCommandRequest, GetApiAuthorData } from "@/src/api/database/types.gen";
...
export function useAuthors() {
    const crud = useCrud<AuthorDto, AuthorInput, UpdateAuthorCommandRequest, GetApiAuthorData["query"]>({
        ...
        update: async (id, command) => {
            const response = await putApiAuthorById({ path: { id }, body: command });
            return (response as unknown as { data: AuthorDetailDto })?.data || response as AuthorDetailDto;
        },
        delete: async id => { await deleteApiAuthorById({ path: { id } }); },
        errorMessages: {
    ...
    const checkNameExists = useCallback(async (name: string, excludeId?: string): Promise<boolean> => {
        try {
            ...
            return items.some(item => item.fullName?.toLowerCase() === name.toLowerCase() && item.id !== excludeId);
        } catch { return false; }
    }, []);
